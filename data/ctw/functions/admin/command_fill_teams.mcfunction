...
## Message for when teams are filled
    execute if score playerNum ctw_setup = playerNum_teams ctw_setup run tellraw @a[tag=ctw_players] [{"text":"NOTE: ","color":"yellow","bold":true},{"text":"An Admin has force filled the teams","color":"red"}]
    execute as @a[tag=ctw_players] at @s run playsound minecraft:entity.enderman.teleport master @s

## Re-enable trigger
    scoreboard players reset @s ctw_fill_teams
    scoreboard players enable @s ctw_fill_teams